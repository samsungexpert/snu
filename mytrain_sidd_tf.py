...
def main(args):
    ...
    model_dir = os.path.join(base_path, 'checkpoint', "generative_" + model_name + model_sig)
    ...
    with strategy.scope():
        ...
        model.save(os.path.join(base_path, 'checkpoint' , f'generative_{model_name}_model_structure.h5'), include_optimizer=False)
        model.summary()
        ...
        callbacks = get_training_callbacks(['ckeckpoint', 'tensorboard', 'image'],
                                            base_path=base_path, model_name= "generative_" + model_name + model_sig,
