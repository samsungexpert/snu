...
class TensorBoardImageCycle(tf.keras.callbacks.Callback):
...
def loss_obj(y_true, y_pred):
    return tf.keras.losses.BinaryCrossentropy(from_logits=True, reduction=tf.keras.losses.Reduction.SUM)(y_true, y_pred)
...
def main(args):
    ...
    model_dir = os.path.join(base_path, 'checkpoint', "cyclegan_" + model_name + model_sig)

    ## dataset
    if args.test:
        data_path = '/home/bw.jung/dataset/SIDD/small/tfrecords'
    ...
    with strategy.scope():
        ...
        bF = GenerationTF(model_name =  model_name,
                          input_shape=(patch_size, patch_size, 3),
                          kernel_regularizer=True,
                          kernel_constraint=constraint_max)
        ...
        model.save_models(os.path.join(base_path, 'checkpoint' ), f'cyclegan_{model_name}_model_structure.h5', include_optimizer=False)
        # model.summaries()
        ...
        tb_dir = os.path.join(base_path, 'board', "cyclegan_" + model_name + model_sig, 'image') #, model_name)
        ...
        ckpt_dir = os.path.join(base_path, 'checkpoint', "cyclegan_" + model_name + model_sig)
        callback_ckpt = SaveModelH5(path=ckpt_dir, name="cyclegan_" + model_name + model_sig, initial_value_threshold=prev_loss)



        callbacks = get_training_callbacks(['tensorboard'],
                                            base_path=base_path, model_name="cyclegan_" + model_name + model_sig,
...
if __name__ == '__main__':
    ...
    parser.add_argument(
            '--data_path',
            type=str,
            default='/home/bw.jung/dataset/SIDD/small/tfrecords',
