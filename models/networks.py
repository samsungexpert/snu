...
class BWUnet(nn.Module):
    def __init__(self, input_nc, output_nc, num_downs, ngf=64, norm='batch', use_dropout=False):
        ...
        tkernel_size=4
        kernel_size=3
        self.dec_tconv4 = nn.ConvTranspose2d(in_channels=ngf*8, out_channels=ngf*8, kernel_size=tkernel_size, stride=2, padding=1)
        self.dec_conv4 = nn.Conv2d(in_channels=ngf*8, out_channels=ngf*4, kernel_size=kernel_size, stride=1, padding=1 )

        self.dec_tconv3 = nn.ConvTranspose2d(in_channels=ngf*4, out_channels=ngf*4, kernel_size=tkernel_size, stride=2, padding=1)
        self.dec_conv3 = nn.Conv2d(in_channels=ngf*4, out_channels=ngf*2, kernel_size=kernel_size, stride=1, padding=1 )
        ...
        self.dec_tconv1 = nn.ConvTranspose2d(in_channels=ngf*1, out_channels=ngf*1, kernel_size=tkernel_size, stride=2, padding=1)
    ...
    def forward(self, x):
        ...
        # last
        dout = self.last(dout12) # 256x256x1*64 -> 256x256x3
        dout = self.tanh(dout)
